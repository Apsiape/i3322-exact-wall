#!/usr/bin/env python3
"""Generate the 25,601-knot Bellman candidate; carries no proof authority."""
...
def main() -> None:
    ...
    report = {
        "status": "25,601-knot candidate only; exact verifier owns the theorem",
        "nodes": NODES,
        ...
        "claim_boundary": (
            ...
            "It proves nothing until exact_refined_witness_threshold.py passes."
        ),
    }
    output = HERE / "refined-bellman-candidate.json"
    output.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({
        key: value for key, value in report.items() if key != "knots_decimal"
    }, indent=2))
